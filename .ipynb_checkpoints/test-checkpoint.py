...
trainer = trainer.trainer(save_dir='./results/',print_if=True,plot_if=True,  
                          save_fig_if=False,save_list_if=False,epoch=10001,reg_name='lap')
trainer.train_dmf(height=height,width=width,
                  shuffle_list=shuffle_list,
                  real_pic=shuffle_pic,mask_in=mask_in,
                  parameters=[height,height],
                  dropout_if=False,nonlinear=None)


'''
import MinPy.train.data_loader as data_loader
import matplotlib.pyplot as plt
import MinPy.net.dmf as dmf
import MinPy.train.trainer as trainer
import torch
parameters=[100,100,100,100]
dmf_net = dmf.net(parameters)

x = torch.eye(parameters[0])
print(dmf_net(x).shape)

y = []
for i in range(parameters[0]):
    y.append(dmf_net(x[:,i]).reshape(-1,1))
y = torch.cat(y,1)
'''
